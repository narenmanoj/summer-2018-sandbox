...
class Generator(nn.Module):
    def __init__(self, opt=None):
        ...
        self.opt = opt
        self.img_shape = (opt.channels, opt.img_size, opt.img_size) if self.opt is not None else (1, 32, 32)
        
        self.latent_dim = opt.latent_dim if self.opt is not None else 100
...
if __name__ == "__main__":
    ...
    for epoch in range(opt.n_epochs):
        ...
        # NSM added these lines ##
        checkpoint_filename = "wgan_gp_mnist_" + str(epochs_done)
        if epochs_done % epoch_interval == 0:
            torch.save(generator.state_dict(), checkpoint_filename)
            print("---checkpoint at epoch %d---" % epochs_done)
        ######
        epochs_done += 1
