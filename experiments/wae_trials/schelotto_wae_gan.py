...
class Encoder(nn.Module):
    def __init__(self, args=None, n_channel=1, dim_h=128, n_z=8):
...
if __name__ == "__main__":
    ...
    linf_bound = 0.50
    clamp_lower = -linf_bound
    clamp_upper = linf_bound
    
    print("clamping everything to lie in [%f, %f]" % (clamp_lower, clamp_upper))
    ...
    for epoch in range(args.epochs):
        ...
        for images, _ in tqdm(train_loader):
            ...
            i1 = -torch.log(d_fake).mean()
            i1.backward()
            i2 = -torch.log(1 - d_real).mean()
            i2.backward()
        ...
        if (epoch + 1) % 1 == 0:
            ...
            save_image(test_data[0].view(batch_size, 1, 28, 28), 
                       './data/reconst_images/wae_gan_clamp_input_%d.png' % (epoch + 1))
            save_image(reconst.data, 
                       './data/reconst_images/wae_gan_clamp_images_%d.png' % (epoch + 1))
