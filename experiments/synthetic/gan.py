...
def is_high_quality(sample, grid_length=5, var=0.0025):
    closest = classification_function(sample)
    dist = np.linalg.norm(sample - closest)
    stdev = np.sqrt(var)
    if -3 * stdev <= dist <= 3 * stdev:
        return True
    return False


def frac_high_quality_samples(loaded_gen,
                              latent_dim=2,
                              grid_length=5,
                              num_gen_samples=20000):
    z = Variable(Tensor(np.random.normal(0, 1, (num_gen_samples, latent_dim))))
    np_samples = loaded_gen(z).cpu().detach().numpy()

    ans = 0.0
    for s in np_samples:
        ans += int(is_high_quality(s, grid_length=grid_length))
    return ans / num_gen_samples


def count_support_size(loaded_gen,
                       latent_dim=2,
                       grid_length=5,
                       num_samples=10000,
                       num_gen_samples=20000):
...
def visualize_latent_space(loaded_gen,
                           latent_dim=2,
                           grid_length=5,
                           num_gen_samples=20000):
    counts = []
    ...
    for i in range(grid_length * grid_length):
        counts.append(0)
        colors.append((np.random.uniform(), np.random.uniform(),
                       np.random.uniform()))
        latent_vecs.append([])
...
class DifferentiableClassifier(nn.Module):
    def __init__(self, grid_length=5):
        ...
        self.model = nn.Sequential(*block(2, 100), *block(100, 100),
                                   *block(100, 100), *block(100, 100),
                                   *block(100, 100), nn.Linear(100, 1))
...
def train_diffable_classifier(grid_length=5,
                              num_epochs=10000,
    ...
    for epoch in range(num_epochs):
        ...
        answer = Variable(
            Tensor(point_to_index(p_np, grid_length=grid_length)),
            requires_grad=False)
        optimizer.zero_grad()
...
def dist_to_boundary(gen,
    ...
    r = Variable(
        Tensor(np.random.normal(0, initial_std, (1, latent_dim))),
        requires_grad=True)
    ...
    for i in range(num_iter):
        if verbose:
            print("[Iteration %d]" % i)
        optimizer.zero_grad()
        loss_val, opt_success_step = class_radius_loss(
            gen, z, r, eps=eps, lamb=lamb)
        if opt_success_step:
            opt_success = True
        loss_val.backward()
    ...
    if not opt_success:
        return Variable(
            Tensor(np.zeros((1, latent_dim))), requires_grad=True), opt_success
    return best_r, opt_success


def class_radius(gen,
                 latent_dim=2,
                 num_trials=1000,
                 num_sub_trials=10,
                 verbose=False,
    ...
    for i in range(num_trials):
        if i % (num_trials // 20) == 0:
            print("Iteration %d" % i)
        z = Variable(
            Tensor(np.random.normal(0, 1, (1, latent_dim))),
            requires_grad=False)
        dist = np.inf
        for j in range(num_sub_trials):
            include_computation = False
            for k in range(num_retrials):
                r, opt_success = dist_to_boundary(
                    gen, z, latent_dim=latent_dim, lr=0.125, verbose=verbose)
                if opt_success:
